...
**TV·전자칠판용 진행 화면은 스크롤 없이 한 화면에 수납**되어야 한다 — 진행에 필요한 버튼(정답 공개·판정·다음)이 1280×800(노트북)과 1920×1080(TV) 모두에서 뷰포트 안에 보여야 한다. 세로 여백·패딩은 `@media (max-height: ...)` 쿼리로 압축한다. (검증 시 두 해상도에서 진행 버튼의 위치를 확인할 것 — D2 연계)
